...
export interface VirtualListCellProps<ItemInfo extends VirtualListCellInfo> extends RX.CommonProps {
    ...
    onLayout?: (itemKey: string, width: number) => void;
...
export class VirtualListCell<ItemInfo extends VirtualListCellInfo> extends RX.Component<VirtualListCellProps<ItemInfo>, RX.Stateless> {
    ...
    private _animatedStyleHeight: RX.Types.AnimatedViewStyleRuleSet;
    ...
    constructor(props: VirtualListCellProps<ItemInfo>) {
        ...
        const leftValue = this._isVisible ? this._left : VirtualListCell._hiddenLeftValue;
        this._leftValue = RX.Animated.createValue(leftValue);

        const topValue = props.top || 0;
        ...
        this._heightValue = RX.Animated.createValue(props.height || 0);

        this._animatedStyleHeight = RX.Styles.createAnimatedViewStyle({
            width: this._heightValue
        });
    }
    ...
    componentWillReceiveProps(nextProps: VirtualListCellProps<ItemInfo>) {
        ...
        if (this.props.height !== nextProps.height) {
            this._heightValue.setValue(nextProps.height);
        }
    ...
    componentDidUpdate(prevProps: VirtualListCellProps<ItemInfo>) {
        ...
        if (this.props.onLayout && this.props.isActive && this._calculatedWidth && this._itemKey) {
            this.props.onLayout(this._itemKey, this._calculatedWidth);
        }
    }
    ...
    setVisibility(isVisible: boolean) {
        if (isVisible !== this._isVisible) {
            ...
            this._leftValue.setValue(this._isVisible ? this._left : VirtualListCell._hiddenLeftValue);
        }
    }
    ...
    setLeft(left: number, animate = false, animationDelay = 0, animationOvershoot = 0) {
        if (left !== this._left) {
            this._left = left;
                ...
                if (animate) {
                    if (animationOvershoot !== 0) {
                        this._topAnimation = RX.Animated.sequence([
                            RX.Animated.timing(this._leftValue, {
                                toValue: left + animationOvershoot,
                                duration: 200,
                        ...
                        this._topAnimation = RX.Animated.timing(this._leftValue, {
                            toValue: left,
                            duration: 200,
                            delay: animationDelay,
    ...
    private _onLayout = (layoutInfo: RX.Types.ViewOnLayoutEvent) => {
        if (this.props.onLayout && this.props.isActive && this._itemKey) {
            this._calculatedWidth = layoutInfo.width;
            this.props.onLayout(this._itemKey, layoutInfo.width);
        }
    }
}
